...
public class MmhScrapeExhaustiveTest {
    ...
    public void exhaustiveTest() {
        ...
        for (String g : List.of("A","B","C","D","E","F")) stats.put(g, new GroupStats(g));
        ...
        for (int fi = 0; fi < 4; fi++) {
            for (int pi = 0; pi < 3; pi++) {
                for (int oi = 0; oi < 3; oi++) {
                    ...
                    results.add(r); stats.get("A").add(r.pass);
        ...
        for (int fi = 0; fi < 4; fi++) {
            for (int pi = 0; pi < 3; pi++) {
                ...
                CaseResult r = run(cid, "B", "https://httpbin.org/image/png", formats[fi], profiles[pi], null);
                results.add(r); stats.get("B").add(r.pass);
                log.info("  [{}] {} - {}", r.pass ? "PASS" : "FAIL", cid, r.briefReason);
            }
        }
        CaseResult r13 = run("B13", "B", "https://httpbin.org/image/png", "markdown", "default", false);
        results.add(r13); stats.get("B").add(r13.pass);
        log.info("  [{}] B13 - {}", r13.pass ? "PASS" : "FAIL", r13.briefReason);
        CaseResult r14 = run("B14", "B", "https://httpbin.org/image/png", "markdown", "default", true);
        results.add(r14); stats.get("B").add(r14.pass);
        ...
        for (int pi = 0; pi < 3; pi++) {
            for (int oi = 0; oi < 3; oi++) {
                ...
                results.add(r); stats.get("C").add(r.pass);
                log.info("  [{}] {} - {}", r.pass ? "PASS" : "FAIL", cid, r.briefReason);
            }
        }
        ...
        for (int fi = 0; fi < 4; fi++) {
            for (int oi = 0; oi < 3; oi++) {
                ...
                results.add(r); stats.get("D").add(r.pass);
                log.info("  [{}] {} - {}", r.pass ? "PASS" : "FAIL", cid, r.briefReason);
            }
        }
        ...
        for (CaseResult r : results) if (!r.pass && r.errorType != null) errStats.merge(r.errorType, 1, Integer::sum);
        log.info("\n## 错误类型统计");
        if (errStats.isEmpty()) log.info("- 无错误");
        else for (Map.Entry<String, Integer> e : errStats.entrySet()) log.info("- {}: {}", e.getKey(), e.getValue());

        List<CaseResult> failed = results.stream().filter(r -> !r.pass).limit(12).toList();
        log.info("\n## 失败样本 (最多 12 条)");
        if (failed.isEmpty()) log.info("- 无失败用例");
        else for (CaseResult r : failed) log.info("- [{}] {}: params={}, reason={}", r.group, r.caseId, r.params, r.briefReason);
        ...
        if (passRate < 100) conclusions.add("⚠ 总体通过率 " + String.format("%.2f", passRate) + "%，存在 " + fail + " 个失败用例");
        for (String ccl : conclusions) log.info(ccl);
        log.info("\n=== 穷举验证完成 ===");
